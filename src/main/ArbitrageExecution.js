...
const ArbitrageExecution = {
  ...
  executeCalculatedPosition(calculated) {
    ...
    logger.execution.info(
      `Attempting to execute ${calculated.id} with an age of ${Math.max(age.ab, age.bc, age.ca).toFixed(
        0,
      )} ms and expected profit of ${calculated.percent.toFixed(4)}%`,
    );

    return ArbitrageExecution.getExecutionStrategy()(calculated)
      .then((actual) => {
        const timeTaken = Util.millisecondsSince(startTime);
        logger.execution.info(
          `${CONFIG.EXECUTION.ENABLED ? "Executed" : "Test: Executed"} ${calculated.id} position in ${Util.millisecondsSince(startTime)} ms`,
        );
        ...
        logger.execution.debug(
          `Expected Conversion:  ${calculated.a.spent.toFixed(8)} ${symbol.a} into ${calculated.b.earned.toFixed(8)} ${symbol.b} @ ${price.ab.expected.toFixed(
            8,
          )}`,
        );
        logger.execution.debug(
          `Observed Conversion:  ${actual.a.spent.toFixed(8)} ${symbol.a} into ${actual.b.earned.toFixed(8)} ${symbol.b} @ ${price.ab.actual.toFixed(8)}`,
        );
        ...
        logger.execution.debug(
          `Expected Conversion:  ${calculated.b.spent.toFixed(8)} ${symbol.b} into ${calculated.c.earned.toFixed(8)} ${symbol.c} @ ${price.bc.expected.toFixed(
            8,
          )}`,
        );
        logger.execution.debug(
          `Observed Conversion:  ${actual.b.spent.toFixed(8)} ${symbol.b} into ${actual.c.earned.toFixed(8)} ${symbol.c} @ ${price.bc.actual.toFixed(8)}`,
        );
        ...
        logger.execution.debug(
          `Expected Conversion:  ${calculated.c.spent.toFixed(8)} ${symbol.c} into ${calculated.a.earned.toFixed(8)} ${symbol.a} @ ${price.ca.expected.toFixed(
            8,
          )}`,
        );
        logger.execution.debug(
          `Observed Conversion:  ${actual.c.spent.toFixed(8)} ${symbol.c} into ${actual.a.earned.toFixed(8)} ${symbol.a} @ ${price.ca.actual.toFixed(8)}`,
        );
        ...
        try {
          Sqlite.insertTradeExecution(
            calculated.id,
            Math.max(age.ab, age.bc, age.ca).toFixed(0),
            calculated.percent.toFixed(4),
            timeTaken,
            calculated.trade.ab.ticker,
            price.ab.expected.toFixed(8),
            price.ab.actual.toFixed(8),
            (((price.ab.actual - price.ab.expected) / price.ab.expected) * 100).toFixed(8),
            calculated.trade.bc.ticker,
            price.bc.expected.toFixed(8),
            price.bc.actual.toFixed(8),
            (((price.bc.actual - price.bc.expected) / price.bc.expected) * 100).toFixed(8),
            calculated.trade.ca.ticker,
            price.ca.expected.toFixed(8),
            price.ca.actual.toFixed(8),
            (((price.ca.actual - price.ca.expected) / price.ca.expected) * 100).toFixed(8),
            symbol.a,
  ...
  isSafeToExecute(calculated) {
    ...
    if (
      Object.entries(ArbitrageExecution.attemptedPositions).find(
        ([executionTime, id]) => id === calculated.id && executionTime > Util.millisecondsSince(CONFIG.EXECUTION.THRESHOLD.AGE),
      )
    ) {
    ...
    if (ArbitrageExecution.getAttemptedPositionsCountInLastSecond() > 1) {
      logger.execution.trace(
        `Blocking execution because ${ArbitrageExecution.getAttemptedPositionsCountInLastSecond()} positions have already been attempted in the last second`,
      );
      return false;
    }
    ...
    return BinanceApi.marketBuyOrSell(calculated.trade.ab.method)(calculated.trade.ab.ticker, calculated.ab.quantity)
      .then((results) => {
        if (results.orderId) {
          ...
          actual.fees += fees;
          recalculated.bc = CalculationNode.recalculateTradeLeg(
            calculated.trade.bc,
            actual.b.earned,
            BinanceApi.getDepthCacheSorted(calculated.trade.bc.ticker),
          );
        }
      ...
      .then((results) => {
        if (results.orderId) {
          ...
          actual.fees += fees;
          recalculated.ca = CalculationNode.recalculateTradeLeg(
            calculated.trade.ca,
            actual.c.earned,
            BinanceApi.getDepthCacheSorted(calculated.trade.ca.ticker),
          );
        }
  ...
  parseActualResults(method, { executedQty, cummulativeQuoteQty, fills }) {
    ...
    const fees = fills.filter((fill) => fill.commissionAsset === "BNB").reduce((total, fill) => total + parseFloat(fill.commission), 0);
